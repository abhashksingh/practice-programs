...
Tree* lookUp( Tree *node, char key)
{
...
Tree *succesorInOrder( Tree *node)
{
...
void pathFinder( Tree *node, int path[], int pathLength)
{
	if(node == NULL) return;
...
/*if the two passed trees are mirror of each other*/
bool isMirror(Tree* left,Tree* right)
{
...
void printLeftView(Tree *node, int level, int *maxlevel)
{
	if(node==NULL)
	{
		return;
...
void printLeftViewStatic(Tree *node, int level)
{
	...
	else
	{
		...
		printLeftViewStatic(node->left,level+1);
		printLeftViewStatic(node->right,level+1);
	}
}
...
int main(int argc, char **argv)
{
	...
	if(found)
	{
		...
		if(succ)
			cout << "In Order Successor of " << ch << " is "<< succesorInOrder(found)->data << endl;
		else 
	...
	if(found)
	{
		...
		if(pred)
			cout << "In Order Predecessor of " << ch << " is "<< predecessorInOrder(found)->data << endl;
		else 
	...
	if(ancestor) 
		cout << "The lowest common ancestor of " << ch1 << " and "<< ch2 << " is " << ancestor->data << endl;
	ch1 = 'E';
	ch2 = 'H';
	ancestor = lowestCommonAncestor(root,lookUp(root,ch1), lookUp(root,ch2));
	if(ancestor) 
	...
	cout << "printing with Left-View" << endl;
	int maxLevel=0;
	printLeftView(newRoot,1,&maxLevel);
	cout<<endl;
	printLeftViewStatic(newRoot,1);
	cout<<endl;
return 0;
}
